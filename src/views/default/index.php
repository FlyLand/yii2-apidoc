    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= $this->context->module->loginConfig['loginUrl']?>" method="post" role="form" class="form-horizontal" id="loginForm">
                    <?php foreach ($this->context->module->loginConfig['fieldMapping'] as $key=>$item) { ?>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9"><input type="text" name="<?= $key;?>" id="loginForm-account" class="form-control"/></div>
                        </div>
                    <?php } ?>
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        <div id="loginErrorText" class="alert alert-danger col-sm-6 col-sm-push-3" style="margin-bottom: 0;display: none;" role="alert">
...
<!--                <li><a href="/wiki/" title="wiki" target="_blank"><b class="text-danger">WIKI</b></a></li>-->
                <li id="loginBtn" <?= !empty($loginInfo)?'style="display:none"':'';?>><a href="#" data-toggle="modal" data-target="#loginModal">用户登录</a></li>
                <li id="accountBtn" <?= empty($loginInfo)?'style="display:none"':'';?> class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown" id="loginInfo-account"><?= !empty($loginInfo['account'])?$loginInfo['account']:'ACCOUNT'?><span class="caret"></span></a>
                    <ul class="dropdown-menu" role="menu">
                        <li><a id="loginInfo-uid"><?= !empty($loginInfo['uid'])?$loginInfo['uid']:'UID'?></a></li>
                        <li><a id="loginInfo-sid"><?= !empty($loginInfo['sid'])?$loginInfo['sid']:'SID'?></a></li>
                        <li class="divider"></li>
                        <li><a href="<?php echo $this->context->module->logoutUrl; ?>" id="logoutBtn">退出登录</a></li>
                    </ul>
                </li>

                <?php foreach ($this->context->module->dropdownList as $item){ ?>
                    <li id="" class="dropdown">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" id=""><?= $item['name']?><span class="caret"></span></a>
                        <ul class="dropdown-menu" role="menu">
        ...
        <div class="panel-group" id="accordion2">
            <?php foreach($controllers as $cname=>$c):?>
                <div class="panel panel-info">
                    <div class="panel-heading" data-toggle="collapse" data-parent="#accordion2" href="#collapse<?= str_replace('\\', '-', $cname)?>">
                        <a href="#" class="accordion-toggle" style="text-decoration:none;display: block;outline: none" title="<?='【controller】'.$cname?>"><?= $module.$c['brief'];?></a>
                    </div>
                    <div id="collapse<?=str_replace('\\', '-', $cname)?>" class="list-group panel-collapse collapse<?= !strcasecmp($controller, $cname) ? ' in': ''?>">
                        <?php
    ...
    <div id="descPannel" class="col-md-5">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <p><span style="display: inline-block;padding-right: 5px;font-weight: bold;">接口功能:</span><?php echo $brief?></p>
